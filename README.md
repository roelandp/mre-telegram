This is a very no frills MRE which enables you to display an interactive Telegram chat wall in AltspaceVR.

In your world you can add this MRE and then a text wall will start pouring in with messages as typed in the selected telegram group.

Also, when an AltspaceVR user clicks on the text-wall a prompt will ask the user for their message. This will get crossposted on Telegram too, as well as on the in-world wall.

There is no persistence (yet all data is stored @ Telegram), so resetting the app will wipe the wall and start waiting for input. Test it by setting up a testgroup.

### You will need to setup a telegram bot
Follow these steps in this order:
- Setup a bot using the @botfather account in Telegram.
- Fill out it's username in `BOT_USERNAME` and its full token in `BOT_TOKEN`.
- Then you will need to turn off "privacy mode" of your bot as well.
- Only once you did all this you should add your bot to the group you want to show in AltspaceVR.
